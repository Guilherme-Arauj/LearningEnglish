...
export class UuidConfig implements IUuidConfig {
  ...
  public async generateProgressVideoId(): Promise<string> {
    const uuid = uuidv4();
    const uuidLimitado = uuid.substring(0, 12);
    const uuidComPrefixo = "V-PROG-" + uuidLimitado;
    return uuidComPrefixo;
  }

  public async generateProgressQuestionId(): Promise<string> {
    const uuid = uuidv4();
    const uuidLimitado = uuid.substring(0, 12);
    const uuidComPrefixo = "Q-PROG-" + uuidLimitado;
    return uuidComPrefixo;
  }
