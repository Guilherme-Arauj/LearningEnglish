...
export class QuestionRepository implements IQuestionRepository {
  ...
  public async create(question: Question): Promise<Question> {
    const created = await this.prisma.question.create({
      data: question.toPersistenceForCreate(),
    });
  ...
  public async update(question: Question): Promise<Question> {
    ...
    const data = question.toPersistenceForUpdate();
  ...
  public async findById(
    ...
    const whereClause = includeDeleted ? { id } : { id, status: "ACTIVE" };
  ...
  public async findByVideoId(videoId: string): Promise<Question[]> {
    const questions = await this.prisma.question.findMany({
      where: {
        videoId: videoId,
        status: "ACTIVE",
  ...
  public async getQuestionsByTimeline(timeline: number): Promise<Question[]> {
    ...
    const quantidadeParaEntregar = Math.floor(questions.length * percentual);
    const questoesParaEntregar = questions.slice(0, quantidadeParaEntregar);

    return questoesParaEntregar.map((q) => this.mapToEntity(q));
  }

  private mapToEntity(prismaQuestion: any): Question {
    return new Question({
      id: prismaQuestion.id,
      videoId: prismaQuestion.videoId ?? undefined,
      ...
      status: prismaQuestion.status || "ACTIVE",
      deletedAt: prismaQuestion.deletedAt ?? undefined,
    });
  }
}
